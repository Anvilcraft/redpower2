...
public class TileTranspose extends TileMachine implements ITubeConnectable {
   ...
   public boolean tubeItemCanEnter(int side, int state, TubeItem item) {
      return side == super.Rotation && state == 2 || side == (super.Rotation ^ 1) && state == 1 && this.buffer.isEmpty() && !super.Powered;
   }
   ...
   public boolean canSuck(int i, int j, int k) {
      if (super.worldObj.getBlock(i, j, k).isSideSolid(super.worldObj, i, j, k, ForgeDirection.getOrientation(super.Rotation))) {
         return false;
      } else {
   ...
   protected AxisAlignedBB getSizeBox(double bw, double bf, double bb) {
      ...
      switch(super.Rotation) {
         case 0:
            return AxisAlignedBB.getBoundingBox(fx - bw, (double)super.yCoord - bb, fz - bw, fx + bw, (double)super.yCoord + bf, fz + bw);
         case 1:
            return AxisAlignedBB.getBoundingBox(fx - bw, (double)(super.yCoord + 1) - bf, fz - bw, fx + bw, (double)(super.yCoord + 1) + bb, fz + bw);
         case 2:
            return AxisAlignedBB.getBoundingBox(fx - bw, fy - bw, (double)super.zCoord - bb, fx + bw, fy + bw, (double)super.zCoord + bf);
         case 3:
            return AxisAlignedBB.getBoundingBox(fx - bw, fy - bw, (double)(super.zCoord + 1) - bf, fx + bw, fy + bw, (double)(super.zCoord + 1) + bb);
         case 4:
            return AxisAlignedBB.getBoundingBox((double)super.xCoord - bb, fy - bw, fz - bw, (double)super.xCoord + bf, fy + bw, fz + bw);
         default:
            return AxisAlignedBB.getBoundingBox((double)(super.xCoord + 1) - bf, fy - bw, fz - bw, (double)(super.xCoord + 1) + bb, fy + bw, fz + bw);
      }
   }
   ...
   protected void suckEntities(AxisAlignedBB bb) {
      ...
      for(Entity ent : (List<Entity>)super.worldObj.getEntitiesWithinAABB(Entity.class, bb)) {
   ...
   public boolean stuffCart(ItemStack ist) {
      ...
      if (!(bl instanceof BlockRail)) {
         ...
         for(EntityMinecartContainer em : (List<EntityMinecartContainer>)super.worldObj.getEntitiesWithinAABB(EntityMinecartContainer.class, this.getSizeBox(0.8, 0.05, 1.05))) {
