   ...
   public static Integer getPressure(World world, WorldCoord wc, int side) {
      ...
      if (te != null) {
         ...
         if (te instanceof IFluidHandler) {
            ...
            FluidTankInfo[] info = ifh.getTankInfo(ForgeDirection.getOrientation(side));
            if (info != null) {
               for(FluidTankInfo i : info) {
                  if (i.fluid != null) {
         ...
         if ((sides & 1 << side) != 0) {
            ...
            if (te != null) {
               if (te instanceof IPipeConnectable) {
                  ...
                  FluidBuffer srcBuffer = src.getPipeBuffer(side);
                  if (srcBuffer != null) {
                     FluidTankInfo[] info = ifh.getTankInfo(ForgeDirection.getOrientation(side ^ 1));
                     if (info != null) {
                        for(FluidTankInfo i : info) {
                           ...
                           if (i.capacity > 0) {
                              if (i.fluid != null) {
                                 if (i.fluid.getFluid() != bType && bType != null) {
                                    continue;
                                 }
                              } else if (bType == null) {
                                 continue;
                              ...
                              int dstPressure = dstLevel <= 0 ? -100 : (int)((double)dstLevel / (double)i.capacity * 100.0);
                              if (srcPressure < dstPressure && dstLevel > 0) {
                                 int qty = Math.min(Math.min(Math.max(25, (dstLevel - srcLevel) / 2), srcBuffer.getMaxLevel() - srcLevel), dstLevel);
                                 if (qty > 0) {
                                    FluidStack drStack = ifh.drain(ForgeDirection.getOrientation(side ^ 1), qty, true);
                                    if (drStack != null) {
                                       srcBuffer.addLevel(drStack.getFluid(), drStack.amount);
                                    }
                                 }
                              } else if (srcPressure > dstPressure && srcLevel > 0) {
                                 int qty = Math.min(Math.min(Math.max(25, (srcLevel - dstLevel) / 2), i.capacity - dstLevel), srcLevel);
                                 if (qty > 0) {
                                    qty = ifh.fill(ForgeDirection.getOrientation(side ^ 1), new FluidStack(bType, qty), true);
