      ...
      private boolean step() {
         ...
         if (wc.y >= 0 && wc.y < this.world.getHeight() - 1) {
            ...
            if (this.movedir >= 0 && !this.world.blockExists(wc.x, wc.y, wc.z)) {
               ...
               IFrameLink ifl = CoreLib.getTileEntity(this.world, wc, IFrameLink.class);
               if (ifl == null) {
                  return true;
                  ...
                  for(int i = 0; i < 6; ++i) {
                     if (ifl.canFrameConnectOut(i)) {
                        ...
                        if (!this.scanmap.contains(sp)) {
                           IFrameLink if2 = CoreLib.getTileEntity(this.world, sp, IFrameLink.class);
                           if (if2 != null) {
                              ...
                              if (this.movedir < 0) {
                                 ...
                                 if (wcls == null || !wcls.equals(this.motorpos)) {
                                    continue;
                                 }
                              }
                           }
         ...
         for(WorldCoord wc : (LinkedList<WorldCoord>)this.frameset.clone()) {
            ...
            if (!this.framemap.contains(sp)) {
               if (!this.world.isAirBlock(wc.x, wc.y, wc.z)) {
                  if (!this.world.canPlaceEntityOnSide(Blocks.stone, sp.x, sp.y, sp.z, true, 0, null, null)) {
                     this.valid = false;
                     return false;
                  }
