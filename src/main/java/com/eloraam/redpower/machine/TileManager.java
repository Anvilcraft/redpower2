...
public class TileManager extends TileMachine implements IBluePowerConnectable, ISidedInventory, ITubeConnectable, ITubeRequest {
   ...
   public int[] getAccessibleSlotsFromSide(int side) {
      return side != super.Rotation && side != (super.Rotation ^ 1) ? IntStream.range(0, 24).toArray() : new int[0];
   }
            ...
            if (inv == null) {
               ...
               for(int i = 0; i < inv.getSizeInventory(); ++i) {
                  ...
                  if (is2 != null && is2.stackSize != 0 && CoreLib.compareItemStack(rq.item, is2) == 0) {
                     if (act) {
                        ItemStack pull = inv.decrStackSize(i, Math.min(rq.item.stackSize, is2.stackSize));
   ...
   public boolean onBlockActivated(EntityPlayer player) {
      if (player.isSneaking()) {
         ...
         if (!super.worldObj.isRemote) {
            player.openGui(RedPowerMachine.instance, 16, super.worldObj, super.xCoord, super.yCoord, super.zCoord);
         }
