...
public class TileItemDetect extends TileMachine implements ITubeConnectable, IInventory, ISidedInventory {
   ...
   public boolean tubeItemCanEnter(int side, int state, TubeItem item) {
      return side == super.Rotation && state == 2 || side == (super.Rotation ^ 1) && state == 1 && this.buffer.isEmpty();
   }
   ...
   public boolean onBlockActivated(EntityPlayer player) {
      if (player.isSneaking()) {
         ...
         if (!super.worldObj.isRemote) {
            player.openGui(RedPowerMachine.instance, 6, super.worldObj, super.xCoord, super.yCoord, super.zCoord);
         }
   ...
   public int[] getAccessibleSlotsFromSide(int side) {
      return side != super.Rotation && side != (super.Rotation ^ 1) ? IntStream.range(0, 9).toArray() : new int[0];
   }
