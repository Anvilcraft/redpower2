...
public class TileMotor extends TileExtended implements IBluePowerConnectable, IRotatable, IFrameLink, IFrameSupport {
   ...
   private void pickFrame() {
      ...
      FrameLib.FrameSolver fs = new FrameLib.FrameSolver(super.worldObj, wc.coordStep(this.Rotation >> 2 ^ 1), wc, this.MoveDir);
      if (fs.solveLimit(RedPowerMachine.FrameLinkSize) && fs.addMoved()) {
         ...
         for(WorldCoord sp : fs.getFrameSet()) {
            ...
            super.worldObj.setBlock(sp.x, sp.y, sp.z, RedPowerMachine.blockFrame, 1, 2);
            super.worldObj.isRemote = ir;
            TileFrameMoving tfm1 = CoreLib.getTileEntity(super.worldObj, sp, TileFrameMoving.class);
            if (tfm1 != null) {
         ...
         for(WorldCoord sp : fs.getFrameSet()) {
            ...
            TileFrameMoving tfm2 = CoreLib.getTileEntity(super.worldObj, sp, TileFrameMoving.class);
   ...
   private void dropFrame(boolean fw) {
      ...
      if (fs.solve()) {
         ...
         FakePlayer player = CoreLib.getRedpowerPlayer(super.worldObj, super.xCoord, super.yCoord, super.zCoord, this.Rotation >> 2, super.Owner);

         for(WorldCoord sp : fs.getFrameSet()) {
            TileFrameMoving ifs = CoreLib.getTileEntity(super.worldObj, sp, TileFrameMoving.class);
            if (ifs != null) {
               ...
               if (ifs.movingTileEntity != null) {
                  ...
                  snapshots.add(new BlockSnapshot(super.worldObj, s2.x, s2.y, s2.z, ifs.movingBlock, ifs.movingBlockMeta, compound));
               } else {
                  snapshots.add(new BlockSnapshot(super.worldObj, sp.x, sp.y, sp.z, ifs.movingBlock, ifs.movingBlockMeta));
               }
            }
         }

         if (!snapshots.isEmpty()
            && !ForgeEventFactory.onPlayerMultiBlockPlace(player, snapshots, ForgeDirection.getOrientation(this.Rotation >> 2 ^ 1)).isCanceled()) {
            for(WorldCoord sp : fs.getFrameSet()) {
               TileFrameMoving ifs = CoreLib.getTileEntity(super.worldObj, sp, TileFrameMoving.class);
               if (ifs != null) {
            ...
            for(WorldCoord sp : fs.getFrameSet()) {
               IFrameSupport frameSupport = CoreLib.getTileEntity(super.worldObj, sp, IFrameSupport.class);
