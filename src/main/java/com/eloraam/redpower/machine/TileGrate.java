...
public class TileGrate extends TileMachinePanel implements IPipeConnectable {
   ...
   public void onBlockPlaced(ItemStack ist, int side, EntityLivingBase ent) {
      super.Rotation = ForgeDirection.getOrientation(this.getFacing(ent)).getOpposite().ordinal();
         ...
         if (this.pressure == 0) {
            ...
            if (this.searchState == 0) {
               ...
               if (this.gratebuf.Type == null) {
                  if (!this.searchPath.startSuck(new WorldCoord(this), 63 ^ 1 << super.Rotation)) {
                     this.restartPath();
                     return;
                  }
               } else {
                  this.searchPath.start(new WorldCoord(this), this.gratebuf.Type, 63 ^ 1 << super.Rotation);
               }
            }
   ...
   public class GratePathfinder {
      ...
      Queue<TileGrate.FluidCoord> allset = new PriorityQueue<>(1024, Collections.reverseOrder());
      ...
      public GratePathfinder(boolean checkVertical) {
         if (checkVertical) {
            ...
            this.workset = new PriorityQueue<>(1024, Comparator.comparingInt(a -> a.dist));
         }
         ...
         for(int i = 0; i < tries; ++i) {
            ...
            if (TileGrate.this.worldObj.isAirBlock(nc.wc.x, nc.wc.y, nc.wc.z)) {
               if (level == this.fluidClass.getDensity()
                  && TileGrate.this.worldObj.setBlock(nc.wc.x, nc.wc.y, nc.wc.z, this.fluidClass.getBlock())) {
                  this.stepAdd(nc);
                  return 0;
               }
            } else if (PipeLib.getFluid(TileGrate.this.worldObj, nc.wc) == this.fluidClass) {
               this.stepAdd(nc);
               int lv1 = this.fluidClass.getDensity(TileGrate.this.worldObj, nc.wc.x, nc.wc.y, nc.wc.z);
               if (lv1 < 1000) {
