...
public class TileRetriever extends TileFilter implements IBluePowerConnectable {
   ...
   public boolean tubeItemEnter(int side, int state, TubeItem item) {
      if (side == (super.Rotation ^ 1) && state == 3) {
         ...
         return side == super.Rotation && state == 2 && super.tubeItemEnter(side, state, item);
      }
   }
   ...
   public boolean tubeItemCanEnter(int side, int state, TubeItem item) {
      if (side == (super.Rotation ^ 1) && state == 3) {
         ...
         return side == super.Rotation && state == 2 && super.tubeItemCanEnter(side, state, item);
      }
   }
   ...
   protected boolean handleExtract(WorldCoord wc) {
      ITubeConnectable itc = CoreLib.getTileEntity(super.getWorldObj(), wc, ITubeConnectable.class);
      if (itc != null && itc.canRouteItems()) {
         if (this.cond.getVoltage() < 60.0) {
            ...
            TubeLib.InRouteFinder irf = new TubeLib.InRouteFinder(super.worldObj, super.filterMap);
            if (this.mode == 0) {
            ...
            if (sm < 0) {
               ...
               if (inv == null) {
                  ...
                  dest.step(side);
                  TileTube tt1 = CoreLib.getTileEntity(super.worldObj, dest, TileTube.class);
                  if (tt1 == null) {
                     return false;
                  } else {
   ...
   public boolean onBlockActivated(EntityPlayer player) {
      if (player.isSneaking()) {
         ...
         if (!super.worldObj.isRemote) {
            player.openGui(RedPowerMachine.instance, 7, super.worldObj, super.xCoord, super.yCoord, super.zCoord);
         }
