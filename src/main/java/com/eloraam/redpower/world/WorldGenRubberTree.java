   ...
   public boolean generate(World world, Random random, int xPos, int yPos, int zPos) {
      ...
      if (yPos >= 1 && yPos + trh + 2 <= world.getHeight()) {
         ...
         for(int var21 = 0; var21 <= 6; ++var21) {
            for(int x = -1; x <= 1; ++x) {
               for(int z = -1; z <= 1; ++z) {
                  world.setBlock(xPos + x, yPos + var21, zPos + z, RedPowerWorld.blockLogs, 1, 3);
               }
            }

            for(int x = -1; x <= 1; ++x) {
               if (random.nextInt(5) == 1 && world.isAirBlock(xPos + x, yPos + var21, zPos - 2)) {
                  world.setBlock(xPos + x, yPos + var21, zPos - 2, Blocks.vine, 1, 3);
               }

               if (random.nextInt(5) == 1 && world.isAirBlock(xPos + x, yPos + var21, zPos + 2)) {
                  world.setBlock(xPos + x, yPos + var21, zPos + 2, Blocks.vine, 4, 3);
               }
            }

            for(int z = -1; z <= 1; ++z) {
               if (random.nextInt(5) == 1 && world.isAirBlock(xPos - 2, yPos + var21, zPos + z)) {
                  world.setBlock(xPos - 2, yPos + var21, zPos + z, Blocks.vine, 8, 3);
               }

               if (random.nextInt(5) == 1 && world.isAirBlock(xPos + 2, yPos + var21, zPos + z)) {
                  world.setBlock(xPos + 2, yPos + var21, zPos + z, Blocks.vine, 2, 3);
               }
            }
         }
         ...
         for(int br = 0; br < nbr; ++br) {
            var24.set((double)random.nextFloat() - 0.5, (double)random.nextFloat(), (double)random.nextFloat() - 0.5);
            var24.normalize();
            double m = ((double)nbr / 10.0 + 4.0) * (double)(1.0F + 1.0F * random.nextFloat());
            var24.x *= m;
            var24.z *= m;
