...
public class RedPowerLib {
    ...
    public static void
    updateIndirectNeighbors(World w, int x, int y, int z, Block block) {
        if (!w.isRemote) {
            for (int a = -3; a <= 3; ++a) {
                for (int b = -3; b <= 3; ++b) {
                    for (int c = -3; c <= 3; ++c) {
    ...
    isBlockRedstone(IBlockAccess iba, int x, int y, int z, int side) {
        switch (side) {
            case 0:
                --y;
                break;
    ...
    public static boolean
    canSupportWire(IBlockAccess iba, int i, int j, int k, int side) {
        switch (side) {
            case 0:
                --j;
                break;
        ...
        if (iba instanceof World) {
            ...
            if (bid.getBlock(i, j, k).isSideSolid(
                    bid, i, j, k, ForgeDirection.getOrientation(side)
                )) {
                return true;
            }
        }
    ...
    public static boolean
    isStrongPoweringTo(IBlockAccess iba, int x, int y, int z, int side) {
    ...
    public static boolean
    isStrongPowered(IBlockAccess iba, int x, int y, int z, int side) {
    ...
    public static boolean
    isWeakPoweringTo(IBlockAccess iba, int x, int y, int z, int side) {
        Block block = iba.getBlock(x, y, z);
        return block != Blocks.air && (!searching || block != Blocks.redstone_wire)
            && (block.isProvidingWeakPower(iba, x, y, z, side) > 0
                || side > 1 && block == Blocks.redstone_wire
                    && block.isProvidingWeakPower(iba, x, y, z, 1) > 0);
    }
    ...
    public static boolean isPoweringTo(IBlockAccess iba, int x, int y, int z, int side) {
        ...
        if (block == Blocks.air) {
            ...
            return side > 1 && block == Blocks.redstone_wire && !searching
                && block.isProvidingWeakPower(iba, x, y, z, 1) > 0;
        }
    }

    public static boolean
    isPowered(IBlockAccess iba, int x, int y, int z, int cons, int inside) {
    ...
    private static int
    getSidePowerMask(IBlockAccess iba, int x, int y, int z, int ch, int side) {
        ...
        if (irp != null) {
            ...
            return isWeakPoweringTo(iba, x, y, z, side)
                ? mask & 16777215
                : (isPoweringTo(iba, x, y, z, side) ? mask : 0);
        }
    }

    public static int
    getPowerState(IBlockAccess iba, int i, int j, int k, int cons, int ch) {
        int trs = 0;
        if ((cons & 17895680) > 0) {
    ...
    public static int
    getRotPowerState(IBlockAccess iba, int i, int j, int k, int rcon, int rot, int ch) {
    ...
    public static int
    getConSides(IBlockAccess iba, int i, int j, int k, int side, int pcl) {
        Block block = iba.getBlock(i, j, k);
        if (iba.isAirBlock(i, j, k)) {
            ...
            if (rpa != null) {
                ...
                return block.canConnectRedstone(iba, i, j, k, getDirToRedstone(side))
                    ? getConDirMask(side)
                    : 0;
            } else {
    ...
    private static int
    getES1(IBlockAccess iba, int i, int j, int k, int side, int pcl, int cc) {
        if (iba.isAirBlock(i, j, k)) {
            return 0;
        } else {
            IConnectable rpa = CoreLib.getTileEntity(iba, i, j, k, IConnectable.class);
    ...
    public static int getExtConSides(
        IBlockAccess iba, IConnectable irp, int i, int j, int k, int dir, int cc
    ) {
    ...
    public static int
    getConnections(IBlockAccess iba, IConnectable irp, int x, int y, int z) {
        int cons = irp.getConnectableMask();
        int cs = 0;
        if ((cons & 17895680) > 0) {
    ...
    public static int
    getExtConnections(IBlockAccess iba, IConnectable irp, int i, int j, int k) {
        byte cs = 0;
    ...
    public static int
    getExtConnectionExtras(IBlockAccess iba, IConnectable irp, int i, int j, int k) {
        byte cs = 0;
    ...
    public static int
    getTileCurrentStrength(World world, int i, int j, int k, int cons, int ch) {
        IRedPowerConnectable irp
            = CoreLib.getTileEntity(world, i, j, k, IRedPowerConnectable.class);
        if (irp == null) {
            return -1;
    ...
    public static int
    getTileOrRedstoneCurrentStrength(World world, int i, int j, int k, int cons, int ch) {
    ...
    private static int
    getIndCur(World world, int i, int j, int k, int d1, int d2, int ch) {
        int d3;
        switch (d1) {
            case 0:
                --j;
        ...
        return getTileCurrentStrength(
            world, i, j, k, 1 << (d4 ^ 1) << ((d3 ^ 1) << 2), ch
        );
    }

    public static int getMaxCurrentStrength(
        World world, int i, int j, int k, int cons, int indcon, int ch
    ) {
        int mcs = -1;
        ...
        if ((cons & 17895680) > 0) {
            mcs = Math.max(
                mcs,
                getTileOrRedstoneCurrentStrength(world, i, j - 1, k, ocon & 35791360, ch)
            );
        }
        ...
        if ((cons & 35791360) > 0) {
            mcs = Math.max(
                mcs,
                getTileOrRedstoneCurrentStrength(world, i, j + 1, k, ocon & 17895680, ch)
            );
        }
        ...
        if ((cons & 71565329) > 0) {
            mcs = Math.max(
                mcs,
                getTileOrRedstoneCurrentStrength(world, i, j, k - 1, ocon & 143130658, ch)
            );
        }
        ...
        if ((cons & 143130658) > 0) {
            mcs = Math.max(
                mcs,
                getTileOrRedstoneCurrentStrength(world, i, j, k + 1, ocon & 71565329, ch)
            );
        }
        ...
        if ((cons & 268452932) > 0) {
            mcs = Math.max(
                mcs,
                getTileOrRedstoneCurrentStrength(world, i - 1, j, k, ocon & 536905864, ch)
            );
        }
        ...
        if ((cons & 536905864) > 0) {
            mcs = Math.max(
                mcs,
                getTileOrRedstoneCurrentStrength(world, i + 1, j, k, ocon & 268452932, ch)
            );
        }
    ...
    public static void updateCurrent(World world, int x, int y, int z) {
        ...
        if (!searching) {
            ...
            while (powerSearch.size() > 0) {
                ...
                IRedPowerWiring sp = CoreLib.getTileEntity(
                    world, c.posX, c.posY, c.posZ, IRedPowerWiring.class
                );
                if (sp != null) {
    ...
    public static int updateBlockCurrentStrength(
        World world, IRedPowerWiring irp, int x, int y, int z, int conm, int chm
    ) {
    ...
    public static boolean isCompatible(int a, int b) {
        return a == b
            || powerClassMapping.contains(new RedPowerLib.PowerClassCompat(a, b));
    }
