...
public class TileLogic extends TileCoverable implements IRedPowerConnectable, IRotatable, IFrameSupport {
   ...
   private void replaceWithCovers(boolean shouldDrop) {
      if (this.Cover != 255) {
         ...
         CoverLib.replaceWithCovers(super.worldObj, super.xCoord, super.yCoord, super.zCoord, 1 << (this.Rotation >> 2 ^ 1), covers);
         if (shouldDrop) {
            CoreLib.dropItem(
               super.worldObj, super.xCoord, super.yCoord, super.zCoord, new ItemStack(this.getBlockType(), 1, this.getExtendedID() * 256 + this.SubId)
            );
         }
   ...
   public void playSound(String name, float f, float f2, boolean always) {
      if (always || RedPowerLogic.soundsEnabled) {
         super.worldObj
            .playSoundEffect((double)((float)super.xCoord + 0.5F), (double)((float)super.yCoord + 0.5F), (double)((float)super.zCoord + 0.5F), name, f, f2);
      }
